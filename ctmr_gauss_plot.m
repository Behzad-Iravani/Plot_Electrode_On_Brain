function a = ctmr_gauss_plot(cortex,electrodes,weights,hemi,viewside)
...
d=a.CLim;
% set(gca,'CLim',[-max(abs(c+eps)) max(abs(c+eps))])
% Behzad
set(gca,'CLim',[-quantile(abs(c),.99)-eps quantile(abs(c),.99)+eps]*1.05)
